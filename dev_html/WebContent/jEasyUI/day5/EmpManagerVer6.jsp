...
<script type="text/javascript">
	function emp_ins(){
		...
		var param="empno="+empno+"&ename="+ename+"&job="+job+"&hiredate="+hiredate+"&sal="+sal+"&comm="+comm+"&deptno="+deptno;
		$.ajax({
			url:'empInsert.jsp'
			,method:'post'
			,data:param
			,success:function(result){
				var tresult = result.trim();
				alert(tresult);
				if(tresult=="성공"){
	...
	function empDEL(){
		...
		var str = $("#dg_emp").datagrid('getChecked');
		var tresult='';
		var count =0;
		for(var i=0;i<str.length;i++){
			var param ="empno="+str[i].EMPNO;
			$.ajax({
				url:'delEMP.jsp'
				,data:param
				,method:"post"
